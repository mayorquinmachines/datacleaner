$(document).ready(function(){
    var dirId = window.location.href.split('/').slice(-1)[0];
    ...
    var getDir = function(tId, cb) {
        $.getJSON('/api/dirs/' + dirId, function( data ) {
    ...
    var updateStreamImg = function() {
        ...
        $('#img-preview').attr('src', '/dir_data/' + dirId + '/' + curFrame + '.png');//+ '_cam-image_array_.jpg');
	...
	var thumnailsOfClip = function(clipIdx) {
        ...
        .map(function(frameIdx) {
            //return '<img class="clip-thumbnail" src="/dir_data/' + dirId + '/' + frames[frameIdx] + '_cam-image_array_.jpg" />';
            return '<img class="clip-thumbnail" src="/dir_data/' + dirId + '/' + frames[frameIdx] + '.png';
        })
        .join('');
